...
## 실행 후 기록: 이 계획서에서 나온 결함 9건의 뿌리

실행 중 계획서 자체의 결함이 9건 나왔다. 전부 코드가 아니라 **검증 절차와 외부 의존성 가정**에서 나왔고, 세 갈래로 정리된다. 다음 계획서를 쓸 때 같은 실수를 반복하지 않기 위해 남긴다.

**1. 검증을 소스가 아니라 실제로 검사할 산출물에 맞춰 쓸 것.**
다섯 건이 이 뿌리를 공유했다 — 압축된 HTML에 `grep -c`(줄 수를 세므로 항상 1), gzip 압축된 Pagefind 조각에 평문 `grep`, `var()`로 남는 값에 계산된 픽셀 문자열 검사, 아직 커밋되지 않은 파일에 `git checkout`, 산문에도 등장하는 문자열을 판별자로 사용.
→ 규칙: 소스와 산출물 사이의 변형(압축·압축해제·CSS 변수 간접·git 인덱스·압축기의 인용부호 재작성)을 명시하고, **스무 개를 쓰기 전에 대표 하나를 실제로 돌려볼 것.** 값이 CSS 커스텀 속성에서 온다면 규칙과 토큰 두 곳을 확인하고 계산된 값은 검사하지 말 것.

**2. 외부 패키지의 API를 기억으로 단정하지 말 것.**
`--pagefind-ui-*`(실제는 `--pf-*`), 존재하지 않는 `param` 옵션, 낡은 폰트 CDN 경로, 빌드 전용 파일을 발행 디렉토리에 두기, Astro 5 가정(실제 7.2.1).
→ 규칙: 옵션 이름을 단정하는 대신 **"설치된 패키지의 타입/CSS를 읽고 그것을 따르라, API가 달라졌으면 보고하라"**로 지시할 것. 실제로 그렇게 지시한 태스크에서는 구현자가 전부 스스로 잡아냈다.

**3. 공유 계약은 한 곳에 한 번만 선언할 것.**
OG 라우트만 초안 필터를 빠뜨린 것, `.narrow`가 파일마다 다른 폭을 의미하게 된 것, 카드 그리드가 3개 파일에 4번 정의된 것 — 전부 같은 뿌리다. 계약에 정해진 집이 없으면 근사 복제가 쌓인다.
→ 규칙: "글을 읽는 모든 소비처는 `getVisiblePosts()`를 통한다"처럼 **불변 조건을 소비처마다 열거하지 말고 한 문장으로 선언할 것.**

**덧붙여**: 픽스처와 검증을 함께 명시하는 계획서는 **픽스처가 그 검증을 실제로 실행시킬 수 있는지** 확인해야 한다. 이 계획서는 관련 글 렌더를 확인하라고 하면서 카테고리별로 글을 1편씩만 뒀기 때문에, 그 경로가 원리상 실행될 수 없었다.
